...
<body class="bg-slate-100 text-slate-800 antialiased font-sans">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const desktopItems = document.querySelectorAll('#desktop-table tbody tr.senior-item');
            const mobileItems = document.querySelectorAll('#mobile-list div.senior-item');
            ...
            const renderView = () => {
                ...
                desktopItems.forEach((el, i) => {
                    ...
                    if (mobileItems[i]) mobileItems[i].style.display = isVisible ? '' : 'none';
                });
                ...
                if (pages > 1) {
                    ...
                    for (let i = 1; i <= pages; i++) {
                        if (pages <= 7 || i === 1 || i === pages || Math.abs(i - currentPage) <= 1) {
                            const activeClass = i === currentPage ? "bg-[#14294D] text-white" : "bg-white text-slate-600 border-slate-300 hover:bg-slate-50";
